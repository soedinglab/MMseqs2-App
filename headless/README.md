...
| [`mcp-server`](mcp-server/) | an MCP server exposing 17 tools over that library, for agents that speak the protocol |
...
cd headless/core && npm test                  # 193 tests, no network
```
...
const table = await (await ticket.wait()).getResult();
console.log(table.getTableSummary({ merged: true }));
```
