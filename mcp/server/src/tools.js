// The twelve tools: name, schema, handler. No MCP SDK here — server.js does the wrapping.
...
export function createTools(operations, { inputDir = null } = {}) {
    ...
    const resolveQuery = async ({ query, queryRef, accession, motif }, { required = true } = {}) => {
        ...
        const loaded = await operations.loadAccession(id, {
            ...
            autoMotif: spec.autoMotif !== undefined ? spec.autoMotif : false,
        });
        // Explicit motifs override accession defaults.
        return { query: loaded.text, motif: motif ?? loaded.motif,
    ...
    const resolveSource = async (from) => {
        ...
        if (type === 'selection') {
            ...
            if (selection.ids.length === 0) throw coded('INVALID_INPUT', `selection "${name}" is empty`);
    ...
    return [
        {
            name: 'get_shared_dir',
            description: 'Verified local shared-directory paths for imports and exported artifacts. ' +
                         'The caller must still confirm that its own session can read the mount.',
...

        {
            ...
            async handler({ ticketId }) {
                ...
                const record = await operations.getTicketRecord(ticketId).catch(() => null);
                return {
                    ticketId,
                    status,
...

        {
            ...
            description: 'Write the complete result — every row, taxonomy node, column — to files and ' +
                         'return their shared-folder locations; nothing is inlined.',
            inputSchema: {
                type: 'object',
                required: ['ticketId'],
                properties: { ticketId: TICKET, queryIdx: QUERY_IDX },
...

        {
            ...
            async handler(args) {
                ...
                if (action === 'copy') {
                    ...
                    const record = await operations.copySelection(ticketId, fromName, name);
...

        {
            ...
            async handler(args) {
                ...
                const selection = existing
                    ?? await operations.selectMsaColumns(ticketId, { entry: entry ?? 0, columns: [], name });
                ...
                if (residues !== undefined) selection.setSubstitutions(residues);
...

        {
            ...
            async handler({ from, tool, ...opts }) {
                ...
                const ticket = Array.isArray(sender.queries)
                    ? await operations.sendQuerySet(sender, { tool, ...opts })
                    : await operations.sendQuery(sender, { tool, ...opts });
                return {
                    ticketId: ticket.id,
                    status: ticket.status ?? 'PENDING',
