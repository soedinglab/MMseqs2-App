...
function packageInfo(input) {
    ...
    const prefix = input.slice(0, at + marker.length);
    return { name, dir: path.resolve(HERE, prefix, ...name.split('/')) };
}

async function thirdPartyNotices(inputs) {
    const packages = new Map(inputs.map(packageInfo).filter(Boolean).map(info => [info.name, info.dir]));
