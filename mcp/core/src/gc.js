...
/** Coordinate one bounded sweep across artifacts, cached results and shared inputs. */
export function createGarbageCollector({
    ...
    onWarning = null,
    audit = fileAudit(path.join(stateDir, 'artifact-gc-audit.jsonl')),
} = {}) {
    const stateFile = path.join(stateDir, 'artifact-gc-state.json');

    return async function collectGarbage({
        minIntervalSeconds = 0, dryRun = false, auditKeeps = false, audit: overrideAudit,
    } = {}) {
        const now = artifactStore.now();
        if (minIntervalSeconds > 0) {
            const last = await fs.readFile(stateFile, 'utf8')
        ...
        const options = {
            audit: overrideAudit ?? audit, dryRun, auditKeeps, now,
        };
        const report = {
            artifacts: await collectArtifacts(artifactStore, options),
            results: await collectResultCache(store, {
                ...options, ttlSeconds: resultTtlSeconds,
            }),
            inputs: importsDir
                ? await collectDroppedInputs(importsDir, {
                    ...options, ttlSeconds: inputTtlSeconds,
                })
                : { skipped: 'NO_SHARED_DIR' },
        };
        await fs.writeFile(stateFile, JSON.stringify({ lastSweepAt: now.toISOString() }))
