...
const normalizeColumns = columns => [...new Set((columns ?? []).map(Number).filter(Number.isInteger))]
    .sort((a, b) => a - b);

export class MsaSelection {
...
export const createMsaSelection = (foldMasonResult, options) => new MsaSelection(foldMasonResult, options);
